...
export default function Programcilar() {
  ...
  const [oneCikanProgramci, setOneCikanProgramcı] = useState(null);
  ...
  const kutlanan = programciListesi.find((programci) => programci.id === oneCikanProgramci)
  return kutlanan.isim;
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: oneCikanProgramci === null ? 'royalblue' : 'gold', // 🤔 kutlarken renk gold'a dönecek
  };
